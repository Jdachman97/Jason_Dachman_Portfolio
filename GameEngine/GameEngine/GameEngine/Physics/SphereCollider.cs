    ...
    public class SphereCollider : Collider
    {
                                   ...
                                   Vector3 lastPosition, out Vector3 normal)
        {
            if (other is SphereCollider)
            {
                ...
                float c = Vector3.Dot(A, A) - (float)Math.Pow((collider.Radius + Radius), 2);
                float disc = (float)Math.Pow(b, 2) - (4 * a * c); // discriminant (b^2 – 4ac)
                if (disc >= 0)
                {
                    float t = (float)(-b - Math.Sqrt(disc) / (2 * a));
